...
export function isArray(info?: string) {
  return Rule((param: any) => Array.isArray(param), info || 'it should be an array')
}

export function isBoolean(info?: string) {
  return Rule((param: any) => [true, false].includes(param), info || 'it should be true or false')
}

export function isNumber(info?: string) {
  return Rule((param: any) => typeof param === 'number', info || 'it should be true or false')
}

export function isString(info?: string) {
  return Rule((param: any) => typeof (param) === 'string', info || 'it should be a string')
}
