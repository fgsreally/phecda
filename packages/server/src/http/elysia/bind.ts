...
export function bind(
  app: App<any>,
  data: Awaited<ReturnType<typeof Factory>>,
  opts: HttpOptions = {},
) {
  const {
    globalGuards,
  ...
  function registerRoute() {
          ...
          return Promise.all(
            body.map((item: any, i) => {
              ...
              return new Promise(async (resolve) => {
                ...
                if (!metaMap.has(tag)) {
                  return resolve(
                    await Context.filterRecord.default(
                      new BadRequestException(`module "${tag}" doesn't exist`),
                    ),
                  )
                }
                ...
                if (!meta) {
                  return resolve(
                    await Context.filterRecord.default(
                      new BadRequestException(`"${method}" in "${tag}" doesn't exist`),
                    ),
                  )
                }
    ...
    for (const [tag, record] of metaMap) {
      for (const method in record) {
        ...
        (subApp as any)[http.method](
          joinUrl(http.prefix, http.route),
          async (c: ElysiaContext) => {
